...
var HandleOutRequest = function(reqId, data, callerIdNum, contextTenant, appType, contextCompany, dvpOriginationType, destNum, domain, callerContext, profile, varUuid, isDialPlanGiven, ctxt, cacheData, eventTime, res)
{
    try
    {
            ...
            if(contextCompany && contextTenant)
            {
                backendFactory.getBackendHandler().GetCacheObject(contextTenant, contextCompany, function(err, cacheInfo)
                {
                    ...
                    else
                    {
                        ...
                        {
                            extDialplanEngine.ProcessExtendedDialplan(reqId, callerIdNum, destNum, callerContext, 'OUT', data, null, contextCompany, contextTenant, null, null, 'inbound', ctxt, cacheData, function(err, extDialplan)
                            {
                                if(err)
                                {
                                                    ...
                                                    if (!err && rule)
                                                    {
                                                        ...
                                                        externalApi.CheckBalance(reqId, varUuid, ep.Origination, ep.Destination, 'minute', ep.Operator, rule.CompanyId, rule.TenantId)
                                                            .then(function(balanceRes)
                                                            {
                                                                ...
                                                                {
                                                                    var xml = xmlBuilder.CreateRouteGatewayDialplan(reqId, ep, callerContext, profile, '[^\\s]*', false, null, 'outbound', rule.Codecs, null);
                                                ...
                                                .then(function(balanceRes)
                                                {
                                                    if (balanceRes && balanceRes.IsSuccess)
                                                    {
                                                        ...
                                                        backendFactory.getBackendHandler().GetCompanyLimits(ctxt.CompanyId, ctxt.TenantId).then(function(compLimits)
                                                        {
                                                            ...
                                                            if(NumLimitInfo)
                                                            {
                                                                var xml = xmlBuilder.CreatePbxFeaturesGateway(reqId, huntDestNum, outRule.ANI, outRule.GatewayCode, ctxt.CompanyId, ctxt.TenantId, null, huntContext, outRule.DNIS, outRule.Operator, outRule.IpUrl, NumLimitInfo, outRule.Codecs, transferCallerName);
                        ...
                        else
                        {
                            if(direction === 'IN')
                            {
                                ...
                                if(appType === 'HTTAPI' && varCompanyId && varTenantId)
                                {
                                    ...
                                    HandleOutRequest(reqId, data, callerIdNum, varTenantId, appType, varCompanyId, dvpOriginationType, destNum, domain, callerContext, profile, varUuid, isDialplanGiven, tempRecCtxt, cacheData, eventTime, res);
                                }
                                else
                                {
