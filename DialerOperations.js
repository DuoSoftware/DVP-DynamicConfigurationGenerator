...
var AgentDial = function(reqId, context, recordingEnabled, extension, customerNumber, companyId, tenantId, csId, agentPubKey, campaignId, res)
{
    backendFactory.getBackendHandler().GetAllDataForExt(reqId, extension, companyId, tenantId, 'USER', csId, null, function(err, extDetails)
    {
        ...
        else
        {
            if (extDetails.SipUACEndpoint && extDetails.SipUACEndpoint.CloudEndUser)
            {
                ...
                var xml = dialplanBuilder.CreateRouteDialerAgentDialplan(reqId, context, '[^\\s]*', attTransInfo, 60, customerNumber, extension, domain, recordingEnabled, tenantId, companyId, agentPubKey, campaignId);
