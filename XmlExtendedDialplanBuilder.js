...
var FaxReceiveUpload = function(reqId, context, destinationPattern, numLimitInfo, dvpCallDirection, companyId, tenantId, trunkNum)
{
    try
    {
        ...
        cond.ele('action').att('application', 'answer')
            ...
            //.up()
            .ele('action').att('application', 'set').att('data', 'api_hangup_hook=api curl_sendfile ' + fileUploadUrl + ' file=${dvpUploadFaxFile} class=CALLSERVER&type=CALL&category=CONVERSATION&referenceid=${uuid}&mediatype=image&filetype=tif&sessionid=${uuid}&display=FAX-' + trunkNum + '-${caller_id_number}')
            .up()
